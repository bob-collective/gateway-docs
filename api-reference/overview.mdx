...
<Steps>
  <Step title="Get Available Routes">
    Call [`GET /v1/get-routes`](/api-reference/get-v-1-get-routes) to fetch all supported routes, chains, and tokens. This helps you understand what swaps are available and present options to your users.
  ...
  <Step title="Get a Quote">
    Call [`GET /v1/get-quote`](/api-reference/get-v-1-get-quote) with your swap parameters (amount, source/destination chains, tokens, etc.). The API returns a quote with routing information, fees, and expected outputs.
  ...
  <Step title="Create an Order">
    Pass the quote to [`POST /v1/create-order`](/api-reference/post-v-1-create-order) to lock in the quote and create an order. This reserves liquidity with the market maker and returns transaction details including:
  ...
  <Step title="Register Transaction">
    Call [`PATCH /v1/register-tx`](/api-reference/patch-v-1-register-tx) with the signed transaction hash/hex. This allows Gateway to track your order and provide status updates.
  ...
  <Step title="Monitor Order Status">
    Track your order's progress using [`GET /v1/get-orders/{user_address}`](/api-reference/get-v-1-get-orders-user-address). This endpoint returns all orders (pending and completed) for a given user address.
